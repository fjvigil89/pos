<?php $is_integrated_credit_sale = is_sale_integrated_cc_processing(); ?>
<!DOCTYPE html>
<html>
	<head>
			...
			<table id="receipt_items">
				<tr>
					...
					<?php if($this->config->item('subcategory_of_items') ){ ?>
							<th class="gift_receipt_element left_text_align" style="width:50%;"><?php echo $this->config->item("custom_subcategory1_name")."/". $this->config->item("custom_subcategory2_name");?></th>
						<?php }?>
				...
				<?php foreach(array_reverse($cart, true) as $line=>$item) { ?>
					<tr>
						<td style="text-align:center;">
							<span class='long_name'><?php echo $item['name']; ?>
								<?php if ($item['size']){ ?> (<?php echo $item['size']; ?>)<?php } ?>
							</span>
						</td>
						<?php if($this->config->item('subcategory_of_items') ){ ?>
								<td class="gift_receipt_element left_text_align">
									<?php if (isset($item['item_id'])){ 
										echo(
											($item['custom1_subcategory']?$item['custom1_subcategory']:"--")."/".
						...
						<?php } ?>
						<td style='<?php echo is_rtl_lang()? 'text-align: left;':'text-align: right;'?>'>
				...
				<?php } ?>
				<tr>
					<td colspan="4" style='<?php echo is_rtl_lang()? 'text-align: left;':'text-align: right;'?>;border-top:2px solid #000000;'>
						<?php echo lang('sales_sub_total'); ?>
					</td>
					<td colspan="2" style='<?php echo is_rtl_lang()? 'text-align: left;':'text-align: right;'?>;border-top:2px solid #000000;'>
				...
				<?php foreach($taxes as $name=>$value) { ?>
					<tr>
						<td colspan="4" style='<?php echo is_rtl_lang()? 'text-align: left;':'text-align: right;'?>'>
							<?php echo $name; ?>:
						</td>
			    ...
			    <?php foreach($payments as $payment_id=>$payment) { ?>
					<tr>
						...
						<td colspan="2" style="<?php echo is_rtl_lang()? 'text-align: left;':'text-align: right;'?>">
							<b>
								<?php echo $this->config->item('round_cash_on_sales') && $payment['payment_type'] == lang('sales_cash') ?  to_currency(round_to_nearest_05($payment['payment_amount'])) : to_currency($payment['payment_amount']); ?> 
							</b> 
						</td>
				    </tr>
				<?php } ?>
				...
				<?php }
					...
					<tr>
						...
						<td colspan="2" style='<?php echo is_rtl_lang()? 'text-align: left;':'text-align: right;'?>'>
							<?php echo to_currency($amount_change * -1); ?>
						</td>
					</tr>	
				<?php } ?>
