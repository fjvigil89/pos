                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="text-center"><span style="cursor: pointer;" class="fa fa-trash-o fa-2x font-red"
                                    onclick="controler('<?php echo site_url() ?>/technical_supports/eliminarArticuloSupport', 'id=<?php echo $row['unique_id']; ?>&uni=<?php echo $row['id']; ?>&nombre=<?php echo $row['nombre']; ?>&idSupport=<?php echo $idSupport ?>', 'refrescar')"></span>
                            </td>
                            <td class="text-center"><?php echo $row['nombre']; ?></td>
                            <td class="text-center">
                                <?php echo to_currency("{$row['precio_con_iva']}", "2"); ?></td>
                            <td width="5"><input type="text" name="cantidad" id="cantidad<?php echo $row['id']; ?>"
                                    value="<?php echo $row['cantidad']; ?>" class="text-center form-control cantidad"
                                    onchange="controler('<?php echo site_url() ?>/technical_supports/updateCantidadSupport','id=<?php echo $row['id']; ?>&idSupport=<?php echo $idSupport ?>&cantidad='+$('#cantidad<?php echo $row['id']; ?>').val(),'refrescar');">
                            </td>
                        ...
                        <tr>
                            <td colspan="5">
                                <h4 class="text-center">
                                    <code><?php echo lang('technical_supports_menj_car'); ?></code></h4>
                            </td>
                        </tr>
                        <?php endif; ?>
                    </tbody>
                    ...
                    <ul class="list-group">
                        <li class="list-group-item">
                            <h5 class="text-center">
                                <?php echo lang('technical_supports_menj_car_prodiva'); ?></h5>
                            <table class="table">
                                <thead>
                                    <tr>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <?php endif; ?>
                                        <td class="text-right">
                                            <?php $total = ($data['precio'] * ($data['iva_total'] / 100)) ?>
                ...
                <div class="portlet light no-padding">
                    ...
                    <li class="list-group-item ">
                        ...
                        <span class="sale-num pull-right">
                            <strong><?php if($abonado!=''){  echo to_currency("{$abonado}", "2"); }else{ $abonado=0;  echo to_currency("{$abonado}", "2"); } ?></strong>
                        </span>
                            ...
                            <ul class="list-group">
                                ...
                                <li class="list-group-item no-border-top">
                                    <span class="name-item-summary">
                                        <i class="fa fa-times-circle fa-lg font-red" class="delete_payment"
                                            onclick="controler('<?php echo site_url() ?>/technical_supports/deletePago', 'payment_id=<?php echo $payment_id ?>&idSupport=<?php echo $idSupport; ?>&precio_total=<?php echo $precio_total ?>', 'contenedor_pago')"></i>
                    ...
                    <div class="add-payment">
                        <form action="javascript:void(0)" method="post" accept-charset="utf-8" id="add_payment_form"
                            autocomplete="off"
                            onsubmit="controler('<?php echo site_url() ?>/technical_supports/facturacion', $('#add_payment_form').serialize(), 'refrescar')">
                            ...
                            <ul class="list-group">
                                <li class="list-group-item no-border-top tier-style"
                                    style="border-top-left-radius: 0px; border-top-right-radius: 0px; border: 0px none;">
                                    <div class="row">
                                        <div class="col-md-6">
                                            <span class="name-addpay">
                                                <a class="sales_help tooltips" data-placement="left" title=""
                                        ...
                                        <div class="col-md-6">
                                            <span class="">
                                                <select name="payment_type" id="payment_types"
                                                    ...
                                                    <option value="Tarjeta de Regalo">Tarjeta de Regalo
                                                    </option>
                                                    <option value="Tarjeta de Débito">Tarjeta de Débito
                                                    </option>
                                        ...
                                        <div class="col-md-12">
                                            <div class="input-group">
                                                <input type="text" name="amount_tendered" value="0.00"
                                                    id="amount_tendered" class="form-control form-inps-sale"
                                                    accesskey="p"> <span class="input-group-btn">
                                                    <input class="btn btn-success" type="button" id="add_payment_button"
                                                        value="Agregar Pago"
                                                        onclick="controler('<?php echo site_url() ?>/technical_supports/agregarPago', $('#add_payment_form').serialize(), 'contenedor_pago')">
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <li class="list-group-item no-border-top tier-style">
                                    <div id="contenedor_submit">
                                        ...
                                        <input type="submit" class="btn btn-success btn-large btn-block"
                                            id="finish_sale_button" value="Completar Venta">
                                        <?php else: ?>
...
$("#item").keyup(function() {
    ...
    if (value == '') {
        ...
        $('#resultado_repuesto').load("<?php echo site_url() ?>/technical_supports/buscarItemsSupport",
            "idSupport=<?php echo $idSupport ?>&buscar=" + value);
    }
    return false;
});
...
var last_total_amount_due =
    <?php echo $this->config->item('round_value') == 1 ? to_currency_no_money(round($saldo_total)) : to_currency_no_money($saldo_total)?>;
