<div class="portlet box light col-xs-12 col-sm-12" >
	...
	foreach ($busqueda->result() as $ListarBusqueda): ?>
		<div class="col-xs-12 col-sm-12" style="padding: 10px;border-bottom: 1px solid #DDDDDD">
			...
			<div class="pull-right bordered"><a class="btn btn-success" href="javascript:void(0);" title="agregar" onclick="controler('<?php echo site_url() ?>/technical_supports/agregarItemSupport','id=<?php echo $ListarBusqueda->item_id ?>&idSupport=<?php echo $idSupport ?>','cart_body');"><i class="fa fa-plus"></i></a></div>
		</div>
	<?php endforeach ?>
