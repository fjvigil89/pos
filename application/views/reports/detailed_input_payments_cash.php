				...
				<div class="portlet-body form">
					...
					<?php echo form_open('',['class'=>'form-horizontal']);?>
						<div class="form-body">
							...
							<div class="form-group">
								<?php echo form_label('<a class="help_config_options  tooltips " data-placement="left" title="'.lang("specific_input_name_help").'">'.lang('reports_customer').'</a>','specific_input_name_label', array('class'=>'col-md-3 control-label')); ?>
								<div class="col-md-9 ">																	
										...
										<?php echo form_input(array(
											...
											"autocomplete"=>"off",
											'class' => 'form-control',
											"placeholder"=>"Comienza por escribir la identificación, nombre o correo",
											'value'=>''));
										?>																
								</div>
							</div>												
	...
	<script type="text/javascript" language="javascript">
		...
		$(document).ready(function()
		{
			...
			$("#generate_report").click(function()
			{
				...
				{
					var url  = window.location+'/'+$("#report_date_range_simple option:selected").val()+ '/' + specific_id +  '/' + export_excel+ '/' + export_pdf;
					if($("#export_pdf_yes").prop('checked'))
						window.open(url);
					else
						window.location =url;
				...
				else
				{
					...
					var url = window.location+'/'+start_date + '/'+ end_date + '/' + specific_id + '/'+ export_excel+ '/' + export_pdf;
					if($("#export_pdf_yes").prop('checked'))
						window.open(url);
					else
						window.location =url;
			...
			$( "#customer_input" ).autocomplete({
				...
				select: function(event, ui)
				{
					...
					$("#specific_input_data").val(ui.item.value);
					return false;
				}
			});
