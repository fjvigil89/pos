				...
				<div class="portlet-body form">
					...
					<?php echo form_open('',['class'=>'form-horizontal']);?>
						<div class="form-body">
							...
							<div class="form-group">
								<?php echo form_label('<a class="help_config_options  tooltips " data-placement="left" title="Número serial a buscar. Dejar vacío para buscar todo">Número serial</a>'.' :', 'reports_sale_type_label', array('class'=>'col-md-3 control-label')); ?> 
								<div class="col-md-9">
									<input id="number-serial" value="" class="form-control"/>
								</div>
							</div>
							<div class="form-group">
							...
							<div class="form-group">
								<?php echo form_label('<a class="help_config_options  tooltips " data-placement="left" title="'.lang("reports_invoice_type").'">'.lang('reports_invoice_type').'</a>'.' :', 'reports_invoice_type', array('class'=>'col-md-3 control-label')); ?> 
								<div class="col-md-9">
									<?php echo form_dropdown('sale_type',array('-1' => 'Todo','1' => 'Factura', '0' => 'Boleta', ), 'all', 'id="sale_ticket" class="form-control"'); ?>
								</div>
							</div>
	...
	<script type="text/javascript" language="javascript">
		...
		$(document).ready(function()
		{
			...
			$("#generate_report").click(function()
			{
				...
				var sale_ticket = $("#sale_ticket").val();
				var numberSerial= $.trim($("#number-serial").val())==""?"all":$("#number-serial").val();
				var export_excel = 0;
				var export_pdf = 0;
				...
				{
					var url = window.location+'/'+$("#report_date_range_simple option:selected").val() + '/'+sale_type+'/'+export_excel+'/'+ export_pdf+'/'+numberSerial+ '/'+sale_ticket;
					if($("#export_pdf_yes").prop('checked'))
						window.open(url);
					else
						window.location =url;
				...
				else
				{
					...
					var url = window.location+'/'+start_date + '/'+ end_date + '/'+sale_type+'/'+ export_excel+'/'+ export_pdf+'/'+numberSerial+'/'+sale_ticket;
					if($("#export_pdf_yes").prop('checked'))
						window.open(url);
					else
						window.location =url;
