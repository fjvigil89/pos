				...
				<div class="portlet-body form">
					...
					<?php echo form_open('',['class'=>'form-horizontal']);?>
						<div class="form-body">
							...
							<div class="form-group">
								<?php echo form_label('<a class="help_config_options  tooltips " data-placement="left" title="'.lang("reports_sale_type_help").'">'.lang('reports_select').'</a>'.': ', 'report',array('class'=>'col-md-3 control-label')); ?>
		                     	<div class="col-md-9">
									<?php echo form_dropdown('reports_select',$reports_select, '', 'id="reports_select" class="form-control"'); ?>
						       </div>	
					       	</div>

					       	<div class="form-group">	
								<?php echo form_label('<a class="help_config_options  tooltips " data-placement="left" title="'.lang("reports_show_manual_adjustments_only_help").'">'.lang('reports_show_manual_adjustments_only').'</a>', 'show_manual_adjustments_only',array('class'=>'col-md-3 control-label')); ?>
								<div class="col-md-9">
	...
	<script type="text/javascript" language="javascript">
		...
		$(document).ready(function()
		{
			...
			$("#generate_report").click(function()
			{
				...
				{
					var url = window.location+'/'+$("#report_date_range_simple option:selected").val() + '/'+show_manual_adjustments_only+'/'+export_excel+'/'+export_pdf+'/'+$("#reports_select  option:selected").val() ;
					if($("#export_pdf_yes").prop('checked'))
						window.open(url);
					else
						window.location =url;
				...
				else
				{
					...
					var url = window.location+'/'+start_date + '/'+ end_date + '/'+ show_manual_adjustments_only + '/'+export_excel+'/'+export_pdf+'/'+$("#reports_select  option:selected").val() ;
					if($("#export_pdf_yes").prop('checked'))
						window.open(url);
					else
						window.location =url;
