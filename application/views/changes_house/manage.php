...
$(document).ready(function() {
    var table_columns = ["invoice_number", "titular_cuenta", 'tipo_documento', 'numero_documento',
        'numero_cuenta', 'divisa', '', ''
    ];
    enable_search('<?php echo site_url("$controller_name/suggest");?>',
        <?php echo json_encode(lang("common_confirm_search"));?>);

    enable_sorting("<?php echo site_url("$controller_name/sorting"); ?>", table_columns,
        <?php echo $per_page; ?>, <?php echo json_encode($order_col);?>,
        <?php echo json_encode($order_dir);?>);
});
</script>
    ...
    <div class="col-md-12">
        ...
        <div class="portlet box green" id="portlet-content">
            <div class="portlet-title">
                ...
                <div class="tools">
                    <span data-original-title="<?php echo $total_rows; ?> total <?php echo $controller_name?>"
                        class="label label-primary tip-left tooltips"><?php echo $total_rows; ?></span>
                </div>
            </div>
            <div class="portlet-body">
                ...
                <div class=" row margin-bottom-10">
                    <?= form_open("$controller_name/search",array('id'=>'search_form','class'=>'form-vertical', 'autocomplete'=> 'off'),array('search_flag'=>1)); ?>
						<div class="col-xs-12 col-md-4 ">
							<div class="input-group">
								<input type="text" title="Ingrese # orden, # cuenta, # documento, nombre clinte, o estado"
									name='search' id='search' class="form-control form-inps"
									value="<?php echo H($search); ?>"
									placeholder="<?php echo lang('common_search')." transacción"?>" />
								<span class="input-group-btn">
									<button name="submitf" class="btn btn-success" type="submit"><i
											class="fa fa-search"></i></button>
								</span>
							</div>
						</div>
						<div class="col-xs-12 col-sm-6 col-md-2 margin-bottom-05">
							<?= form_dropdown('estado', $options, $estado, 'id="estado" class="bs-select form-control"'); ?>
						</div>
						<?php if($this->Employee->has_module_action_permission('changes_house', 'refuse_approve', $this->Employee->get_logged_in_employee_info()->person_id)) :?>
							<div class="col-xs-12 col-sm-6 col-md-4 margin-bottom-05">
								<?= form_dropdown('empleado_id', $empleados, $estado, 'id="empleado_id" class="bs-select form-control"'); ?>
							</div>
						<?php endif;?>
						<div class="col-xs-12 col-sm-6 col-md-2 margin-bottom-05">
							<?=form_dropdown('report_date_range_simple',$report_date_range_simple, '0000-00-00/0000-00-00', 'id="report_date_range_simple" class="bs-select form-control"'); ?>
						</div>
						<div class="col-xs-12 col-sm-6 col-md-3 col-lg-2 margin-bottom-05">
							<div class="input-group input-daterange" id="reportrange">
								...
								<input type="text" value="<?php echo $start_date; ?>" class="form-control start_date"
									name="start_date" id="start_date" placeholder="Selecciona una fecha y hora">
							</div>
						</div>
						<div class="col-xs-12 col-sm-6 col-md-3 col-lg-2  margin-bottom-05">
							<div class="input-group input-daterange" id="reportrange1">
								...
								<input type="text" class="form-control end_date" value="<?php echo $end_date; ?>"
									name="end_date" id="end_date" placeholder="Selecciona una fecha y hora">
							</div>
						</div>
						<div class="col-xs-12 col-sm-6 col-md-2 margin-bottom-05">
						...
						<div class="col-xs-12 col-sm-6 col-md-3 col-lg-3 margin-bottom-05 ">
							<a href="<?php echo site_url($controller_name."/clear_state"); ?>"
								class="btn btn-info btn-block clear-state pull-right effect"><?php echo lang('common_clear_search'); ?></a>
						</div>
					</form>
                </div>
                <div class="row">
...
<script type="text/javascript">
	...
	var url = "<?= site_url("warehouse/last_orders")?>";
