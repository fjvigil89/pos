...
class Summary_items extends Report
{
	...
	{		
		$columns = array();
		$columns[] = array('data'=>lang('reports_imagen'), 'align'=> 'left');
		$columns[] = array('data'=>lang('reports_item'), 'align'=> 'left');
		$columns[] = array('data'=>lang('items_colour'), 'align'=> 'left');
		$columns[] = array('data'=>lang('items_model'), 'align'=> 'left');
		$columns[] = array('data'=>lang('items_marca'), 'align'=> 'left');
	...
	public function getData()
	{
		...
		$this->db->select('items.name,items.marca, items.colour, items.model,items.image_id,items.item_number, items.product_id, items.category, location_items.quantity , sum(quantity_purchased) as quantity_purchased, sum(subtotal) as subtotal, sum(total) as total, sum(tax) as tax, sum(profit) as profit', false);
