...
class Detailed_payments extends Report
{
	...
	public function getData()
	{
		...
		$this->db->select('sale_id, SUM(total) as total,is_invoice,ticket_number,invoice_number', false);
		...
		$this->db->select('sales.sale_time,sales.invoice_number,sales.is_invoice,sales.ticket_number, sales_payments.sale_id, sales_payments.payment_type, payment_amount, payment_date, payment_id', false);
		...
		//Si el metodo de pago es igual a "Todos" me trae todos los registros de lo contrario filtra segun el tipo de pago
		if ($this->params['payment_type'] == lang('sales_all')) 
		{
			$sales_payments = $this->db->get()->result_array();
		}
		else
		{	
			$this->db->where('sales_payments.payment_type',urldecode($this->params['payment_type']));
