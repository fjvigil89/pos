...
class Specific_employee extends Report
{
	...
	public function getData()
	{
		...
		$this->db->select('sale_id, sale_time, register_name, sale_date, sum(quantity_purchased) as items_purchased, CONCAT(first_name," ",last_name) as customer_name, sum(subtotal) as subtotal, sum(total) as total, sum(tax) as tax, sum(profit) as profit, payment_type, is_invoice,ticket_number,invoice_number,comment', false);
		...
		if (!empty($this->params['payment_type']))
		{
			$string = @ereg_replace('%20', ' ', trim($this->params['payment_type']));
	        $text=preg_replace("/[<>]/", '_', $string);
			$this->db->like('payment_type', $text);
		}
