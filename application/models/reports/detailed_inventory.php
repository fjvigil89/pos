...
class Detailed_inventory extends Report
{
	...
	{
		if ($this->config->item('show_image')==1 && !$this->params['export_excel']) 
		{
			return array(
			array('data'=>lang('reports_date'), 'align' => 'left'),
		   	array('data'=>lang('reports_imagen'), 'align' => 'left'),
	...
	public function getData()
	{
		...
		if ($this->params['reports_select']==0)
		{
			$this->db->where('trans_inventory >', 0);
		}
      	if ($this->params['reports_select']==1)
        {
			$this->db->where('trans_inventory <', 0);
		}
