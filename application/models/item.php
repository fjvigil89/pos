...
class Item extends CI_Model
{
	...
	function expired_products_subcategory(){
		//$location_id=$this->Employee->get_logged_in_employee_current_location_id();
		$now = date('Y-m-d');	
		$day = $this->config->item("subcategory_alerts") ? $this->config->item("subcategory_alerts"): 15 ;
		$new_data = date("Y-m-d",strtotime($now."+ ".$day." days"));
		$between = 'between ' . $this->db->escape($now . ' 00:00:00').' and ' . $this->db->escape($new_data . ' 23:59:59');
