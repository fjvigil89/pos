...
class Item_taxes extends CI_Model
{
	...
	*/
	function save(&$items_taxes_data, $item_id)
	{
		...
		if (count($current_taxes) != count($items_taxes_data))
		{
			...
			if($row['percent']!==NUll || $row['name']=='Impuesto1' )
			{


				if (empty($row['percent'])  &&  $row['name']=='Impuesto1')
				{
					$row['percent']=0;
				}
	...
	*/
	...
	function get_all_offline($date)
	{
		$this->db->from('items_taxes');
		$this->db->where('update_item_taxes  > ',$date);
		return $this->db->get();
	}
		function count_all()
	{
	...
	function get_taxes_tax_row($taxes_id,$item_id)
	{
	    

	$this->db->from('items_taxes');
    $this->db->where(array('id '=>$taxes_id, 'item_id'=>$item_id));
