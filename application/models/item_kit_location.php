...
{
	function get_tier_prices_all(){

		$this->db->select('location_item_kits_tier_prices.*');
		$this->db->from('location_item_kits_tier_prices');
		$this->db->join('item_kits', 'item_kits.item_kit_id = location_item_kits_tier_prices.item_kit_id');
	...
	function get_all($date="0000-00-00 00:00:00"){
		...
		$this->db->select('location_item_kits.*');
		$this->db->from('location_item_kits');
		$this->db->join('item_kits', 'item_kits.item_kit_id = location_item_kits.item_kit_id ');
		if($date=="0000-00-00 00:00:00"){
			$this->db->where($this->db->dbprefix('item_kits').".deleted",0);
		}
		else{
			$this->db->where('update_location_item_kits` > ',$date);
