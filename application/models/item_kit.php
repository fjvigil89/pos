...
{
	function get_tier_price_row_all()
	{
		$this->db->select('item_kits_tier_prices.*');
		$this->db->from('item_kits_tier_prices');
		$this->db->join('item_kits', 'item_kits.item_kit_id = item_kits_tier_prices.item_kit_id');
		$this->db->where($this->db->dbprefix('item_kits').".deleted",0);
		return $query = $this->db->get();
		
	}
	function get_all_indexddb($date)
	{		
		$this->db->from('item_kits');
		if($date=="0000-00-00 00:00:00"){
			$this->db->where('deleted',0);
		}else{
			$this->db->where('update_kit > ',$date);
		}	
		return $this->db->get();
	}
	/*
