	...
	*/
	...
	{	
	 ...
	 function get_db(){
		...
		$login_db->where('store_name !=','inpos_demo2017');
		$login_db->where('store_name !=','inpos_test1');
		$login_db->where('store_name !=','inpos_prueba');
		$login_db->where('license_type !=','demo');	

		$login_db->where('FROM_unixtime(UNIX_TIMESTAMP(expire_date),"%Y-%m-%d") >=', date('Y-m-d'));	
