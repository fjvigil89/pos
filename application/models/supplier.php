...
{	
	...
	*/
	...
	function exists_suppliers_items($item_id,$person_id)
	{
	...
	*/
	...
	function save_suppliers($item_data_supliers)
	{

		
		if (!$item_data_supliers['supplier_id'] or !$this->exists_suppliers_items($item_data_supliers['item_id'],$item_data_supliers['supplier_id']))
		{
			$success=$this->db->insert('items_suppliers',$item_data_supliers);
		}
		else
		{

		$this->db->where('supplier_id', $item_data_supliers['supplier_id']);
		$this->db->where('item_id', $item_data_supliers['item_id']);
		$success= $this->db->update('items_suppliers',$item_data_supliers);
		}
		return $success;
	}

	function suppliers_info($item_id) 
	{
			
		$this->db->from('items_suppliers');
		$this->db->join('suppliers', 'items_suppliers.supplier_id = suppliers.person_id');		
		$this->db->where('items_suppliers.item_id',$item_id);
		$this->db->order_by("company_name", "asc");

		return $this->db->get()->result();	
	}
	function suppliers_info_id($item_id, &$supplier_id)
	{
	...
	*/
	...
	function delete_suppliers($item_id,$supplier_id)
	{
		
		return $this->db->delete('items_suppliers', array('supplier_id'=>$supplier_id,'item_id' => $item_id));
	}
