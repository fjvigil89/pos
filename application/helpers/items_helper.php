...
function get_price_for_item_including_taxes_quotes($item_id_or_line, $item_price_excluding_tax, $quote_id = FALSE)
{
	$return = FALSE;
	$CI =& get_instance();
	if ($quote_id !== FALSE)
	{
		$tax_info = $CI->Sale->get_sale_items_taxes_quotes($quote_id,$item_id_or_line);
	}	
	else
	{
		$tax_info = $CI->Item_taxes_finder->get_info($item_id_or_line);
