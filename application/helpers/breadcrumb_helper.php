...
function create_breadcrumb()
{
	...
	if ($ci->uri->segment(1) == 'customers')
	{
		...
		elseif ($ci->uri->segment(2) == 'redeempoints') 
		{
			$return.=create_current_page_url(lang('customers_redeempoints'));
		}
  	}
	elseif($ci->uri->segment(1) == 'items')
	{
		...
		elseif($ci->uri->segment(2) == 'excel_import_compare')
		{
			$return.=create_current_page_url(lang('items_excel_compare'));
		}
	}
	...
	elseif($ci->uri->segment(1) == 'reports')
	{
		...
		elseif ($ci->uri->segment(2) == 'detailed_payment')
		{
		...
		elseif($ci->uri->segment(2) == 'defective_items_log')
		{
			$return.=create_report_breadcrumb(lang('reports_defective_item_report'));
		}
		elseif($ci->uri->segment(2)=="tables_report"){
			$return.=create_report_breadcrumb(lang('tables_report'));

		}
		elseif($ci->uri->segment(2)=="detailed_report_table"){
			$return.=create_report_breadcrumb(lang('detailed_report_table'));
	...
	elseif ($ci->uri->segment(1) == 'registers_movement')
	{
		if ($ci->uri->segment(2) == false) //Main page
		{
			$registers_movement_home_link =create_current_page_url(lang('module_registers_movement'));
		}
		else
		{
			$registers_movement_home_link = '<a href="'.site_url('registers_movement').'">'.lang('module_registers_movement').'</a>';
		}
		
		$return.=$registers_movement_home_link;
				
		if($ci->uri->segment(2) == 'view')
		{
			if ($ci->uri->segment(3) == '')
			{
				$return.=create_current_page_url(lang('cash_flows_deposit_money'));
			}			
			else if ($ci->uri->segment(3) == 0)
			{
				$return.=create_current_page_url(lang('cash_flows_xtract_money'));
			}
		}
	}
	...
	elseif ($ci->uri->segment(1) == 'sales' && $ci->uri->segment(2) == 'quoteses')
		
	{
		{
			$sales_home_link = '<a href="'.site_url('sales').'">'.lang('sales_quotes').'</a>';
	...
	elseif ($ci->uri->segment(1) == 'campaigns')
	{
		$sales_home_link = '<a href="'.site_url('campaigns').'">'.lang('module_campaigns').'</a>';
		$return.=$sales_home_link;		
	}
