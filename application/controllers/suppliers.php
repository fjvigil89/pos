...
class Suppliers extends Person_controller
{
		...
		else
		{
			if (($handle = fopen($_FILES['file_path']['tmp_name'], "r")) !== FALSE)
			{
				...
				for($k = 2;$k<=$num_rows; $k++)
				{
					...
					$type = $sheet->getCellByColumnAndRow(13, $k)->getValue();
					if (!$type)
					{
						$type = '';
					}				
					...
					if ($supplier_data['type'] == 1) 
					{
						$supplier_data['type'] = lang('suppliers_natural');
					}
					elseif ($supplier_data['type'] == 2) 
					{
						$supplier_data['type'] = lang('suppliers_legal');
					}										
	...
	*/
	function save($supplier_id=-1)
	{
		...
		'type'=> $this->input->post('suppliers_type'),
