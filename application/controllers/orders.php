...
class Orders extends Person_controller
{
	...
	{
		$params = $this->session->userdata('orders_search_data') ? $this->session->userdata('orders_search_data') : array('offset' => 0, 'order_col' => 'last_name', 'order_dir' => 'asc', 'search' => FALSE);
	...
	*/
	function view($customer_id=-1,$redirect_code=0)
	{
		...
		$data['controller_name']=strtolower(get_class());
		$data['tiers']=$tiers;
		$data['person_info']=$this->Order->get_info($customer_id);
		$data['person_info_point']=$this->Order->get_info_points($customer_id);
		$data['redirect_code']=$redirect_code;
		$this->load->view("customers/form",$data);
	}
	function redeempoints($customer_id=-1,$redirect_code=0)
	{
		$this->check_action_permission('add_update');
		$data['controller_name']=strtolower(get_class());
		$data['person_info']=$this->Order->get_info($customer_id);
		$data['person_info_point']=$this->Order->get_info_points($customer_id);
		$data['redirect_code']=$redirect_code;
		$this->load->view("customers/redeempoints",$data);
	}
	...
	{
		if($this->Order->account_number_exists($this->input->post('account_number')))
		echo 'false';
		else
		echo 'true';
	...
	*/
	function save($customer_id=-1)
	{
		...
		if (is_numeric($this->input->post('value_point')))
		{
           $this->load->model('points');
           $points=$this->input->post('value_point');
           $this->points->update_point($points,$customer_id);
		}
		
		if($this->Order->save($person_data,$customer_data,$customer_id))
		{
	...
	*/
	...
	{
		echo json_encode(array('duplicate'=>$this->Order->check_duplicate($this->input->post('term'))));
	}
	/* added for excel expert */
	function excel_export() {
		$data = $this->Order->get_all($this->Order->count_all())->result_object();
		$this->load->helper('report');
		$rows = array();
		...
		else
		{
			if (($handle = fopen($_FILES['file_path']['tmp_name'], "r")) !== FALSE)
			{
				...
				{					
					...
					if(!$this->Order->save($person_data,$customer_data, $person_id ? $person_id : FALSE))
					{	
