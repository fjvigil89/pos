...
function do_delete(url)
{
	...
	$.post(url, { 'ids[]': row_ids },function(response)
	{
		...
		if(response.success)
		{
			toastr.success(response.message, COMMON_SUCCESS);
		...
		{
			toastr.error(response.message, COMMON_ERROR);			
		}
	...
	$('#select_all').click(function()
	{
		...
		{	
			...
			$("#sortable_table tbody :checkbox").each(function()
			{
				...
				$(this).parent().parent().find("td").addClass('selected').css("backgroundColor","#F5F5F5");
		...
		else
		{
			...
			$("#sortable_table tbody :checkbox").each(function()
			{
				...
				$(this).parent().parent().find("td").removeClass('selected').css("backgroundColor","");				
			});    	
		}
	 });	
}
...
function enable_row_selection(rows)
{
	...
	rows.click(function row_click(event)
	{
		...
		{
			$(this).find("td").addClass('selected').css("backgroundColor","#F5F5F5");
		}
		else
		{
