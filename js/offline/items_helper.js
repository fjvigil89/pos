class Items_helper {
	...
	async  get_commission_for_item(item_id, price, quantity,discount)
	{
	...
	else if(item_info.commission_percent > 0)
	{
	...
	async get_price_for_item_excluding_taxes(item_id_or_line, item_price_including_tax, sale_id = false) {
		var return_ = false;
		var tax_info = null
		//if (sale_id != false) {
		...
		//}
		//else {
		tax_info = await objItem_taxes_finder.get_info(item_id_or_line);
		//}
		if (tax_info.length == 2 && tax_info[1].cumulative == 1) {
			return_ = item_price_including_tax / (1 + (tax_info[0].percent / 100) + (tax_info[1].percent / 100) + ((tax_info[0].percent / 100) * ((tax_info[1].percent / 100))));
		}
		else //0 or more taxes NOT cumulative
		{
			...
			return_ = item_price_including_tax / (1 + (total_tax_percent / 100));
		}
		if (return_ != false) {
